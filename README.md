# @shapeshift-labs/frontier-lang-python

Python projection adapter for Frontier Lang semantic documents.

## Usage

```js
import { emitPythonWithSourceMap } from '@shapeshift-labs/frontier-lang-python';

const { code, sourceMap, ast } = emitPythonWithSourceMap(document, {
...
These are Frontier-only package measurements for @shapeshift-labs/frontier-lang-python. They exercise the package's own parser, checker, compiler, projection, CLI, fuzz, or semantic-kernel surface without making competitor comparison claims.
